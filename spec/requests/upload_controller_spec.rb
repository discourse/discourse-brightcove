...
RSpec.describe Brightcove::UploadController do
  ...
  let!(:upload_request_stub) do
    stub_request(:get, "https://ingest.api.brightcove.com/v1/accounts/987654321/videos/12/upload-urls/discourse_12_filename.mp4").
      to_return(status: 200, body: { secret_access_key: "secret", api_request_url: "https://my.domain/video.mp4", bucket: "Bucket" }.to_json, headers: {})
  end

  let!(:ingest_request_stub) do
    stub_request(:post, "https://ingest.api.brightcove.com/v1/accounts/987654321/videos/12/ingest-requests").
  ...
  context "while logged in" do
    ...
    it "allows signing" do
      ...
      expect(response.status).to eq(200)
      expect(response.body).to eq("36e5f9fa782ba7a7932fd8e21a4343a1c462e4f98aaa81674ea72fd8d0596edc")
    end

    it "allows ingest" do
      v = Brightcove::Video.create!(video_id: "12", secret_access_key: "abcd", state: "pending", api_request_url: "https://hello.world/video.mp4")
  ...
  context "with login_required" do
    ...
    let!(:video) { Brightcove::Video.create!(video_id: "12", secret_access_key: "abcd", state: "pending", api_request_url: "https://hello.world/video.mp4", callback_key: SecureRandom.hex) }
    it "updates status upon callback" do
      post "/brightcove/callback/#{video.video_id}/#{video.callback_key}", params: { entityType: "TITLE", action: "SOMETHINGELSE", status: "SUCCESS" }.to_json, headers: { "CONTENT_TYPE" => "application/json" }
      expect(response.status).to eq(200)
      video.reload
      expect(video.state).to eq("pending")

      post "/brightcove/callback/#{video.video_id}/#{video.callback_key}", params: { entityType: "TITLE", action: "CREATE", status: "ERROR" }.to_json, headers: { "CONTENT_TYPE" => "application/json" }
      expect(response.status).to eq(200)
      video.reload
      expect(video.state).to eq("errored")

      post "/brightcove/callback/#{video.video_id}/#{video.callback_key}", params: { entityType: "TITLE", action: "CREATE", status: "SUCCESS" }.to_json, headers: { "CONTENT_TYPE" => "application/json" }
      expect(response.status).to eq(200)
      video.reload
